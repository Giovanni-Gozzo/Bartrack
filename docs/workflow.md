...
L'utilisateur s'inscrit puis s'authentifie pour récupérer son identité via Token JWT.
...
Avant de s'entraîner sérieusement, l'athlète s'étalonne sur différents exercices.
- **`POST /rm1_users/`** : Il renseigne un 1 Rep Max s'il le connaît.
- **`POST /initialize`** : Initialisation scientifique du profil avec 2 combinaisons de vitesses et RPE (le backend calcule son *slope* et *intercept* interne).
...
## 4. La Séance d'Entraînement 🏋️‍♂️
...
- **`POST /daily_1rm/`** (Calcul Optionnel) : Après sa première montée en charge, l'app évalue le `e1RM` du jour basé sur sa vélocité de chauffe !
